# -*- coding: utf-8 -*-
"""
Created on Fri Jul  5 10:30:24 2019
...
p_num=np.arange(1,6)
parameters = [{'n_estimators':range(10,11,1),"max_depth":range(3,4
               )}]
r_xgb = XGBClassifier()
clf = GridSearchCV(r_xgb, parameters,n_jobs=4,cv=10)
clf.fit(x_train,y_train)  
print('The parameters of the best model are:------ ')
print(clf.best_params_)
'''
#model=svm.SVC(kernel="linear",C=0.95)  #线性核函数,不包含K折交叉验证
#model=XGBClassifier(n_neighbors=clf.best_params_["n_neighbors"],weights=clf.best_params_['weights'],p=clf.best_params_["p"])  
model = XGBClassifier(n_neighbors=20,max_depth=5,learning_rate=0.1)
...
filename = r'F:\岩石基因图谱\二次重做\尖晶石\y_test.csv'
with open(filename ,'w')as f:
    for number in y_test:
        f.write(str(number)+'\n')
        
#将#将rbf_svr_y_predict写入文件
filename__ = r'F:\岩石基因图谱\二次重做\尖晶石\y_predict.csv'
...
#保存模型
import pickle
with open(r"F:\岩石基因图谱\二次重做\尖晶石\knn_","wb") as f:
    pickle.dump(model,f)
